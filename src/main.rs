			...
			match events.next()? {
				...
				Key::Char('d') => {
					if show_actions { 
						...
						let result = git_branch_delete(&branch_name, &false);
						let output_buff = match result {
						  DeleteResult::Ok(output) => {
								bl.remove_selected();
								// print to command bar
								output
							},
						  DeleteResult::NotFullyMerged(output) => {
								output
							},
						  DeleteResult::Err(output) => output
						};
						command_bar_text = Some(output_buff);
					}
				}
				// shift+d means force delete
				Key::Char('D') => { 
					if show_actions { 
						let is_head_detached = false; // TODO
						if bl.is_current_selected() || is_head_detached {
						...
						let branch_name = bl.get_selected_branch_name();
						let result = git_branch_delete(&branch_name, &true);
						let output_buff = match result {
						  DeleteResult::Ok(output) => {
								bl.remove_selected();
								// print to command bar
						  ...
						  DeleteResult::NotFullyMerged(output) => output,
						  DeleteResult::Err(output) => output
						};
						command_bar_text = Some(output_buff);
					}
...
pub fn git_branch_delete(branch_name: &str, force: &bool) -> DeleteResult {
	...
	if output.status.success() {
		...
		DeleteResult::Ok(stdout)
	} else {
		let stderr: String = String::from_utf8(output.stderr).unwrap();
		if stderr.contains(strings::NOT_FULLY_MERGED) {
			DeleteResult::NotFullyMerged(stderr)
		} else {
			DeleteResult::Err(stderr)
		}
	}
}
